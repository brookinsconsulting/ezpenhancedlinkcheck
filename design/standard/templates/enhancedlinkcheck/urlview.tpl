...
<div class="block">
<form method="post" action={concat( 'enhancedlinkcheck/edit/', $url_object.id, '?offset=', ezgetvars()['offset'],'&limit=', ezgetvars()['limit'], '&sSearch=', ezgetvars()['sSearch'] )|ezurl} target="_blank">
...
<form action={concat( 'enhancedlinkcheck/urlview/', $url_object.id )|ezurl} method="post">
...
{let object_version_status=$Objects.status}
...
<tr class="{$Objects.sequence}">
    ...
    <td><a href={concat( 'content/edit/', $Objects.item.contentobject_id )|ezurl} target="_blank"><img src={'edit.gif'|ezimage} alt="{'Edit'|i18n( 'design/admin/url/view' )}" title="{'Edit <%object_name>.'|i18n( 'design/admin/url/view',, hash( '%object_name', $Objects.item.name ) )|wash}" /></a></td>
</tr>
{/if}
